...
//class  STMMapperCache{}

class STMMapperCache[V <: LongKeyedMapper[V] with IdPKP](mediatorName: String, myType: V, idExtractor: V => Long) extends LogHelper {
  ...
  def cacheGetById(iid: Long, returnOriginals: Boolean = false): Option[V] = {
    atomic { implicit txn =>
      ...
      val olocal = cache.get.getById(iid)
          ...
          if (isTesting == false) {
            ...
            odbobj.isDefined match {
              case true => //We have it in the database
                cache.get.insert(odbobj.head) //Cache it first
  ...
  def addSearchField(fieldName: String, extractor: V => Any) {
    atomic { implicit txn =>
      cache.get.addSearchField(fieldName, extractor)
    }
  }
  ...
  def findAll(fieldName: String, fieldValue: Any, returnOriginals: Boolean = false): List[V] = {
    atomic { implicit txn =>
      ...
      val fn = cache.get.findAll(fieldName, fieldValue)
      returnOriginals match {
        case false => fn.map(cloneObject(_))
        case true => fn
      }
  ...
  def cacheAndDbUpdate(v: V) {
    atomic { implicit txn =>
      ...
      cache.get.updateSearchFields(local)
  ...
  def cacheAndDbDelete(v: V) {
    atomic { implicit txn =>
      ...
      val local = cache.get.getById(v.id.get).get
      ...
      cache.get.delete(local) //delete from cache
    }
  }
  ...
  def cacheAndDbReloadAll() {
    atomic { implicit txn =>
      //log.trace("cacheAndDbReloadAll before: " + cache.get.idCache.size)
      cacheDeleteAll()
      //log.trace("After cache clean: " + cache.get.idCache.size)
      if (isTesting == false) { //If this is not a test save it
  ...
  protected def updateLocalFromRemote(remote: V): V = {
    atomic { implicit txn =>
      if (useClones == true) { //If we use clones, we must copy all the data from the cloned objects to the original objects
        val local = cache.get.getById(remote.id.get).get
        //traceObject(local)
        ...
        cache.get.update(remote) //I DO NOT understand why but if we do not use clones this should be done, even though another thread updated the cached object directly
        return remote
      }
    }
  }
